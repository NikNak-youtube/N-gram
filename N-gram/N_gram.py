...
def loop_text(text):
    ...
    for i in range(4, len(words)-1):
        key = (words[i-4].lower().strip(nochar), words[i-3].lower().strip(nochar), words[i-2].lower().strip(nochar), words[i-1].lower().strip(nochar), words[i].lower().strip(nochar))
...
# make a new generate_text function that allows for larger values of n
...
def generate_text(word_list, length=1000):
    ...
    for i in range(length):
        print(current_key[4], end=' ')
        current_key = (current_key[1].lower(), current_key[2].lower(), current_key[3].lower(), current_key[4].lower().strip(nochar))
        next_words = word_list.get(current_key, None)
        if not next_words:
            break
        next_word = random.choice(next_words)
        current_key = (current_key[1].lower().strip(nochar), current_key[2].lower().strip(nochar), current_key[3].lower().strip(nochar), current_key[4].lower().strip(nochar), next_word)
